...
test_name 'C57 - Configure Puppet Enterprise as Broker'
...
verify_broker_type_regex = /broker-type: puppet-pe/
...
on(razor_server, create_broker_command) do |result|
  assert_match(verify_broker_name_regex, result.stdout, 'Broker incorrectly configured!')
  assert_match(verify_broker_type_regex, result.stdout, 'Broker incorrectly configured!')
  assert_match(verify_broker_server_regex, result.stdout, 'Broker incorrectly configured!')
end
