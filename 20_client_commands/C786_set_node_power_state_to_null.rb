...
test_name 'C786 Set Node Power State to "null"'
step 'https://testrail.ops.puppetlabs.net/index.php?/cases/view/786'
...
razor agents, 'register-node', json do |node, text|
  ...
  json = {"name" => nodeid, "to" => nil}
  razor node, 'set-node-desired-power-state', json do |node, text|
    assert_match /set desired power state to ignored/, text
  end
end
